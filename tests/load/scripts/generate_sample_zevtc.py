...
blob. arcdps-tooling convention (per
  ``libs/gw2_evtc_parser/src/gw2_evtc_parser/parser.py::HEADER_SIZE = 25``):

  bytes  0-3:   magic b"EVTC" (4 chars)
  bytes  4-11:  build version, 8 ASCII chars (e.g. b"20251001")
  byte  12:     rev (uint8, 0 for legacy)
  bytes 13-14:  combat_id (uint16, 0 for empty)
  byte  15:     unused (uint8)
  bytes 16-19:  agent_count (uint32, 0 here)
  bytes 20-23:  skill_count (uint32, 0 here)
  byte  24:     lang (uint8, 0 here)

Total header: 25 bytes.
No agents, no skills, no event stream. arcdps writes a zip wrapper
around a single EVTC entry named ``fight.evtc``; the upload route does
``zipfile.is_zipfile(data) -> ZipFile(data).read(first_entry)`` so we
wrap our EVTC in a single-entry zip with zipfile.ZIP_STORED.
...
def _build_minimal_evtc_header() -> bytes:
    """Build the 25-byte arcdps EVTC header."""
    # Zero agents/skills; total inner payload is 25 bytes.
