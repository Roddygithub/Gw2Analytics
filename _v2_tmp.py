...
def read_events(stem, inst):
    ...
    evs = sorted([e for e in events if getattr(e,"skill_id",None)==718 and getattr(e,"target_agent_id",None)==aid and type(e).__name__=="BoonApplyEvent"], key=lambda e:e.time_ms)
    return origin, evs
...
class SI:
    ...
    def extend(s,v,src): s.ext.append((src,v))
...
for inst,exp in ((3089,44.757),(3232,43.852)):
    origin,evs=read_events(stem,inst)
    ...
    for pl in d['players']:
        if pl.get('instanceID')==inst:
            for bu in pl.get('buffUptimes',[]):
                if bu.get('id')==718:
                    print("  EI:",round(bu['buffData'][0]['uptime'],3),bu['buffData'][0]['wasted'],bu['states'])
