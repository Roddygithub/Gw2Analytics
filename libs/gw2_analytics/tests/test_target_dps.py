...
class TestTargetDpsAggregator:
    ...
    def test_check_invariants_raises_on_sum_mismatch(self) -> None:
        rows = [
            TargetDpsRow.model_construct(
                target_agent_id=1, total_damage=100, attack_count=1, dps=10.0
            ),
        ]
        with pytest.raises(ValueError, match=r"sum of row\.total_damage"):
            TargetDpsAggregator._check_invariants(rows, expected_sum=200)

    def test_check_invariants_raises_on_attack_count_below_one(self) -> None:
        rows = [
            TargetDpsRow.model_construct(
                target_agent_id=1, total_damage=100, attack_count=0, dps=10.0
            ),
        ]
        with pytest.raises(ValueError, match=r"attack_count.*must be >= 1"):
            TargetDpsAggregator._check_invariants(rows, expected_sum=100)
    ...
    def test_check_invariants_raises_on_wrong_ordering(self) -> None:
        rows = [
            ...
            TargetDpsRow.model_construct(
                target_agent_id=2, total_damage=200, attack_count=1, dps=20.0
            ),
        ]
        with pytest.raises(ValueError, match=r"not ordered by"):
            TargetDpsAggregator._check_invariants(rows, expected_sum=300)

    def test_check_invariants_raises_on_tie_not_broken(self) -> None:
        rows = [
            TargetDpsRow.model_construct(
                target_agent_id=2, total_damage=100, attack_count=1, dps=10.0
            ),
            TargetDpsRow.model_construct(
                target_agent_id=1, total_damage=100, attack_count=1, dps=10.0
            ),
