...
class TestPlayerBoonsAggregator:
    ...
    def test_check_invariants_raises_on_boons_out_sum_mismatch(self) -> None:
        rows = [
            PlayerBoonsRow.model_construct(
                agent_id=1,
                boons_out=100,
                boons_in=0,
                boons_out_rate=0.0,
        ...
        with pytest.raises(ValueError, match=r"sum of row\.boons_out"):
            PlayerBoonsAggregator._check_invariants(
                rows, expected_total_out=200, expected_total_in=0
            )

    def test_check_invariants_raises_on_partition_mismatch(self) -> None:
        rows = [
            PlayerBoonsRow.model_construct(
                agent_id=1,
                boons_out=2,
        ...
        with pytest.raises(ValueError, match=r"fixed_sum.*\+.*other_sum.*!= boons_out"):
            PlayerBoonsAggregator._check_invariants(rows, expected_total_out=2, expected_total_in=0)
    ...
    def test_check_invariants_raises_on_wrong_ordering(self) -> None:
        ...
        with pytest.raises(ValueError, match=r"not ordered by"):
            PlayerBoonsAggregator._check_invariants(
                rows, expected_total_out=300, expected_total_in=0
            )
