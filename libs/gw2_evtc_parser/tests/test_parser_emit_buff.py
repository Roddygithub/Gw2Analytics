...
def test_parse_events_2025_marks_life_leech_from_buff_cycle() -> None:
    evtc = _build_minimal_evtc(
        [(1, 1, 1, "Src", True)],
        build="20250925",
        skills=[(30285, "Vampiric Aura")],
        events=[
            _build_event_record_2025(
                time_ms=10_000,
                src_agent=1,
    ...
    assert len(events) == 1
    assert isinstance(events[0], DamageEvent)
    assert events[0].is_life_leech is True


def test_parse_events_2025_marks_condition_damage_against_downed_from_offcycle() -> None:
    evtc = _build_minimal_evtc(
        [(1, 1, 1, "Src", True)],
        build="20260612",
        skills=[(736, "Bleeding")],
    ...
    assert len(events) == 1
    assert isinstance(events[0], DamageEvent)
    assert events[0].against_downed is True


def test_parse_events_2025_keeps_pre_may_condition_against_downed_on_pad() -> None:
    evtc = _build_minimal_evtc(
        [(1, 1, 1, "Src", True)],
        build="20251123",
        skills=[(736, "Bleeding")],
...
def test_parse_events_2025_keeps_blocked_damage_attempt() -> None:
    evtc = _build_minimal_evtc(
        [(1, 1, 1, "Src", True)],
        build="20250925",
        skills=[(101, "Attack")],
