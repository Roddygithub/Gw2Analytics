...
Usage:
    uv run python scripts/ei-parity/ei_diff.py [--json out.json] [log-stem ...]

The local WvW corpus is a normal development input. It is gitignored and must
never be published, but requires no executor, token, or special runtime.
"""
...
def validate_corpus() -> tuple[list[str], str, dict[str, object]]:
    ...
    if len(corpus) != len(set(corpus)) or len(corpus) != 35:
