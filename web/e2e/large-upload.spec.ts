...
for (const filePath of LARGE_PATHS) {
  ...
  test(`large .zevtc upload bypasses Next.js 10 MB proxy limit: ${fileLabel}`, async ({ page }) => {
    ...
    test.skip(
      ...
      `${fileLabel} is only ${sizeBytes} bytes — need >10 MiB to exercise the proxy bypass`,
    );
    ...
    await page.screenshot({ path: `${SHOTS}/large-upload-bypass-done-${fileLabel}.png`, fullPage: true });
  });
}
