 ...
 *
 * The page calls 3 fetchers via ``Promise.allSettled``:
 ...
 *
 * The mock server returns fixture JSON for all 3 endpoints when
 * the fight id is ``fixture-fight-001`` or ``fixture-fight-002``.
 *
 * Why a single smoke test (vs 5 + 1 for each section)
 * ==================================================
 * The 5 roll-up sections (per-target damage + healing +
 * buff-removal + per-subgroup + per-skill) all share the same
 * SSR fetch path; the section-level rendering is fully covered
 * by the unit tests in tests/app/fight-events-page.test.tsx.
 * The E2E only needs to prove that the SSR pipeline returns a
 * fully-rendered tree for a real fight id (so a regression in
 * the fetchers, the routing, or the page contract is caught).
 */
